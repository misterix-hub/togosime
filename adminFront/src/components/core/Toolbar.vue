<template>
  <v-toolbar
    ...
    <v-toolbar-title>
      <v-toolbar-side-icon @click.prevent="toggleNavigationBar"></v-toolbar-side-icon>
    </v-toolbar-title>
    <!-- <v-text-field
      flat
      solo-inverted
    ...
    <span style="font-size: 25px">{{ $store.state.curentUser.username }}</span>
...
export default {
      ...
      items: [
        {
          ...
          click: (e) => {
            ...
            vm.$router.push({ name: 'users_profile' });
          }
        },
        /* {
          icon: 'account_circle',
  ...
  computed: {
    photo (){
      if(this.$store.state.curentUser.photo != null || this.$store.state.curentUser.photo != ''  ){
        return this.axios.defaults.baseURL+this.$store.state.curentUser.photo
      }else{
  ...
  methods: {
    toggleNavigationBar() {
      ...
      window.getApp.toggleNavigationBar();
    },
