          ...
          <v-alert type="info" dismissible v-model="alert.showInfo" transition="scale-transition">
            ...
            <v-card >
              <v-toolbar card color="white">
                ...
                ></v-text-field>
                <v-btn depressed outline icon fab dark @click.native="getGammes()" color="indigo" small>
                  <v-icon>refresh</v-icon>
                </v-btn>
                <v-btn  depressed outline icon fab dark @click.native="$router.push({name: 'product_gamme_add'})" color="indigo" small >
              ...
              <v-card-text class="pa-0">
                <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                    <td><span style="cursor: pointer" @click="$router.push({ name: 'product_gamme_edit', params: {id: props.item.id} })"> {{ props.item.nom }}   </span></td>
                    <td>  {{ props.item.description }}   </td>
                    <td>
                      <v-btn  depressed outline icon fab dark color="primary" small @click.native="$router.push({ name: 'product_gamme_edit', params: {id: props.item.id} })">
...
export default {
  ...
  methods: {
    removeClient () {
      ...
      this.axios.delete('api/gammes/' + this.elementRemove+'/delete',).then((response) => {
