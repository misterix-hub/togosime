          ...
          <v-alert type="info" dismissible v-model="alert.showInfo" transition="scale-transition">
            ...
            <v-card >
              <v-toolbar card color="white">
                ...
                <v-btn  depressed outline icon fab dark @click.native="$router.push({name: 'product_category_add'})" color="indigo" small >
              ...
              <v-card-text class="pa-0">
                <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                    ...
                    <td>  <span v-if="props.item.gamme_produit">{{ props.item.gamme_produit.nom }}</span><span v-else>---</span>   </td>
                    <td>
                      <v-btn  depressed outline icon fab dark color="primary" small @click.native="$router.push({ name: 'product_category_edit', params: {id: props.item.id} })">
...
export default {
  ...
  methods: {
    removeCategorie () {
      ...
      this.axios.delete('api/categories/' + this.elementRemove+'/delete',).then((response) => {
