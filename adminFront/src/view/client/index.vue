          ...
          <v-alert type="info" dismissible v-model="alert.showInfo" transition="scale-transition">
            ...
            <v-card >
              <v-toolbar card color="white">
                ...
                ></v-text-field>
                <v-btn depressed outline icon fab dark @click.native="getClients()" color="indigo" small>
                  <v-icon>refresh</v-icon>
                </v-btn>
                <v-btn  depressed outline icon fab dark @click.native="$router.push({name: 'client_add'})" color="indigo" small >
              ...
              <v-card-text class="pa-0">
                <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                    <td><span style="cursor: pointer" @click="$router.push({ name: 'client_edit', params: {id: props.item.id} })">{{ props.item.identifiant }}</span></td>
                    ...
                    <td>
                      <v-btn  depressed outline icon fab dark color="primary" small @click.native="">
...
export default {
  ...
  methods: {
    removeClient () {
      ...
      this.axios.delete('api/clients/' + this.elementRemove,).then((response) => {
    ...
    getClients () {
      ...
      this.axios.get('api/partenaires/'+vm.$store.state.curentUser.partenaire_id+'/clients').then((response) => {
