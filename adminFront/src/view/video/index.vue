          ...
          <v-alert type="info" dismissible v-model="alert.showInfo" transition="scale-transition">
            ...
            <v-card >
              <v-toolbar card color="white">
                ...
                ></v-text-field>
                <v-btn depressed outline icon fab dark @click.native="getVideos()" color="indigo" small>
                  <v-icon>refresh</v-icon>
                </v-btn>
                <v-btn  depressed outline icon fab dark @click.native="$router.push({name: 'video_add'})" color="indigo" small >
              ...
              <v-card-text class="pa-0">
                <v-data-table
                  ...
                  <template slot="items" slot-scope="props">
                    <td><span style="cursor: pointer" @click="$router.push({ name: 'video_edit', params: {id: props.item.id} })">  {{ props.item.titre }}  </span> </td>
                    <td>  {{ props.item.description }}   </td>
                    <td>  {{ props.item.url }} </td>
                    <td>
                      <v-btn  depressed outline icon fab dark color="primary" small @click.native="$router.push({ name: 'video_edit', params: {id: props.item.id} })">
    ...
    <v-dialog v-model="basic2.dialog" persistent max-width="500px">
      <v-card color="red" style="color: #FFFFFF">
        ...
        <v-card-actions>
          ...
          <v-btn color="white" flat @click.native="basic2.dialog = false, removeVideo()">Oui</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    removeVideo () {
      ...
      this.axios.delete('api/videos/' + this.elementRemove,).then((response) => {
